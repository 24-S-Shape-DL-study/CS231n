...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    # For numerical stability, shift scores by subtracting max score for each training example
    ...
    loss = -np.sum(np.log(correct_class_probs)) / num_train  # Average loss
    loss += reg * np.sum(W * W)  # Regularization

    # Compute the gradient
    dS = probs.copy()
    dS[np.arange(num_train), y] -= 1  # Shape: (N, C)
    dW = X.T.dot(dS) / num_train  # Shape: (D, C)
