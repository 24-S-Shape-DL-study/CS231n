...
def affine_forward(x, w, b):
    ...
    # 입력 데이터 x를 (N, D) 크기의 2차원 배열로 변형
    X_reshaped = x.reshape(x.shape[0], -1) 
    
    # Affine 변환
    out = X_reshaped.dot(w) + b  # (N, D) dot (D, M) + (M,) = (N, M)
...
def affine_backward(dout, cache):
    ...
    dw = X_reshaped.T.dot(dout)  # (D, N) dot (N, M) = (D, M)
    
    db = np.sum(dout, axis=0) 
    
    dx = dout.dot(w.T).reshape(x.shape)  # (N, M) dot (M, D) -> (N, D) then reshape to original shape
...
def softmax_loss(x, y):
    ...
    correct_class_scores = softmax[np.arange(x.shape[0]), y]
